...
| :--- | :--- | :--- | :--- |
| `/api/v1/tenants` | `GET/POST` | Lista o crea empresas (Tenants) | ⏳ Pendiente |
| `/api/v1/tenants/{id}`| `PUT/DEL` | Edita o elimina un Tenant | ⏳ Pendiente |
| `/api/v1/users` | `GET/POST` | Lista o crea usuarios | ⏳ Pendiente |
| `/api/v1/users/{id}` | `PUT/DEL` | Edita o elimina un usuario | ⏳ Pendiente |

### 📊 Dashboard e Importación
| Endpoint | Método | Descripción | Estado |
| :--- | :--- | :--- | :--- |
| `/api/v1/dashboard/stats` | `GET` | Devuelve métricas consolidadas (Deuda total, activos, riesgo) | ⏳ Pendiente |
...
| `/api/ai/debtors/{id}/payments` | `POST` | Registra una promesa de pago acordada por la IA | ✅ Listo |
| `/api/ai/debtors/{id}/interactions`| `POST` | Guarda el log (historial/resumen) de la conversación | ✅ Listo |
| `/api/ai/notifications/email` | `POST` | Envía un recordatorio por correo a petición de la IA | ✅ Listo |
